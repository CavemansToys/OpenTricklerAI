...
bool http_rest_cleanup_mode_state(struct fs_file *file, int num_params, char *params[], char *values[]) {
    ...
    for (int idx = 0; idx < num_params; idx += 1) {
        if (strcmp(params[idx], "s0") == 0) {
            ...
            // Validate state enum (0-1 based on cleanup_mode_state_t)
            if (state_value < CLEANUP_MODE_EXIT || state_value > CLEANUP_MODE_ENTER) {
                return send_validation_error(file, "Invalid cleanup mode state");
            }
            cleanup_mode_state_t new_state = (cleanup_mode_state_t) state_value;
            ...
            float trickler_speed = strtof(values[idx], NULL);
            validation = validate_cleanup_speed(trickler_speed);
            if (!validation.is_valid) {
                return send_validation_error(file, validation.error_message);
            }
            cleanup_mode_config.trickler_speed = trickler_speed;
            motor_set_speed(SELECT_BOTH_MOTOR, cleanup_mode_config.trickler_speed);
        }
    }
