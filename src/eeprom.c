...
// Improved random number generator using multiple entropy sources
uint32_t rnd(void){
    int k;
    uint32_t random = 0;
    ...
    // This helps reduce predictability after system reset
...
void eeprom_register_handler(eeprom_save_handler_t handler) {
    ...
    if (new_node == NULL) {
        // Out of memory - critical error, cannot register handler
        // In embedded system, this is a fatal condition
        printf("FATAL: Out of memory in eeprom_register_handler\n");
        return;  // Handler won't be registered but system continues
    }
