...
bool http_rest_neopixel_led_config(struct fs_file *file, int num_params, char *params[], char *values[]) {
    ...
    for (int idx = 0; idx < num_params; idx += 1) {
        if (strcmp(params[idx], "bl") == 0) {
            ...
            int chain_count = atoi(values[idx]);
            validation = validate_led_chain_count(chain_count);
            if (!validation.is_valid) {
                return send_validation_error(file, validation.error_message);
            }
            neopixel_led_config.eeprom_neopixel_led_metadata.pwm_out_led_chain_count = (neopixel_led_chain_count_t)chain_count;
        }
            ...
            int colour_order = atoi(values[idx]);
            validation = validate_led_colour_order(colour_order);
            if (!validation.is_valid) {
                return send_validation_error(file, validation.error_message);
            }
            neopixel_led_config.eeprom_neopixel_led_metadata.pwm_out_led_colour_order = (neopixel_colour_order_t)colour_order;
        }
    ...
    // Response
    int len = snprintf(neopixel_config_json_buffer,
    ...
    CHECK_SNPRINTF_OVERFLOW(len, sizeof(neopixel_config_json_buffer), file);
