...
bool http_rest_profile_config(struct fs_file *file, int num_params, char *params[], char *values[]) {
    ...
    validation_result_t validation = validate_profile_index(profile_idx);
    ...
    bool save_to_eeprom = false;
        ...
        for (int idx = 0; idx < num_params; idx += 1) {
            if (strcmp(params[idx], "p0") == 0) {
                ...
                float value = strtof(values[idx], NULL);
                validation = validate_pid_kp(value);
                if (!validation.is_valid) {
                    return send_validation_error(file, validation.error_message);
                }
                current_profile->coarse_kp = value;
            }
            else if (strcmp(params[idx], "p4") == 0) {
                float value = strtof(values[idx], NULL);
                validation = validate_pid_ki(value);
                if (!validation.is_valid) {
                    return send_validation_error(file, validation.error_message);
                }
                current_profile->coarse_ki = value;
            }
            else if (strcmp(params[idx], "p5") == 0) {
                float value = strtof(values[idx], NULL);
                validation = validate_pid_kd(value);
                if (!validation.is_valid) {
                    return send_validation_error(file, validation.error_message);
                }
                current_profile->coarse_kd = value;
            }
            else if (strcmp(params[idx], "p6") == 0) {
                float value = strtof(values[idx], NULL);
                validation = validate_flow_speed(value);
                if (!validation.is_valid) {
                    return send_validation_error(file, validation.error_message);
                }
                current_profile->coarse_min_flow_speed_rps = value;
            }
                ...
                current_profile->coarse_max_flow_speed_rps = value;
            }
                ...
                current_profile->fine_min_flow_speed_rps = value;
            }
                ...
                current_profile->fine_max_flow_speed_rps = value;
            }
            else if (strcmp(params[idx], "p13") == 0) {
                current_profile->ai_tuning_enabled = string_to_boolean(values[idx]);
            }
        ...
        int len = snprintf(buf, sizeof(buf),
                          "%s"
                          "{\"pf\":%d,\"p0\":%ld,\"p1\":%ld,\"p2\":\"%s\",\"p3\":%0.3f,\"p4\":%0.3f,\"p5\":%0.3f,\"p6\":%0.3f,\"p7\":%0.3f,\"p8\":%0.3f,\"p9\":%0.3f,\"p10\":%0.3f,\"p11\":%0.3f,\"p12\":%0.3f,\"p13\":%s}",
                          http_json_header,
                          profile_idx,
                          current_profile->rev,
...
bool http_rest_profile_summary(struct fs_file *file, int num_params, char *params[], char *values[])
{
    ...
    for (uint8_t p_idx=0; p_idx < MAX_PROFILE_CNT; p_idx+=1) {
        len = snprintf(&buf[char_idx], sizeof(buf) - char_idx,
                      item_template,
                      p_idx, &profile_data.profiles[p_idx].name);
        if (len < 0 || len >= (int)(sizeof(buf) - char_idx)) {
