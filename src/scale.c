...
bool scale_init() {
    ...
    // Mutex to protect current_scale_measurement from concurrent access
    scale_config.scale_measurement_mutex = xSemaphoreCreateMutex();
...
// Thread-safe setter for scale measurement (used by scale drivers)
void scale_set_current_measurement(float value) {
    BaseType_t scheduler_state = xTaskGetSchedulerState();
    if (scheduler_state != taskSCHEDULER_NOT_STARTED) {
        xSemaphoreTake(scale_config.scale_measurement_mutex, portMAX_DELAY);
    }
    scale_config.current_scale_measurement = value;
    if (scheduler_state != taskSCHEDULER_NOT_STARTED) {
        xSemaphoreGive(scale_config.scale_measurement_mutex);
    }
}
...
float scale_get_current_measurement() {
    ...
    measurement = scale_config.current_scale_measurement;
    if (scheduler_state != taskSCHEDULER_NOT_STARTED) {
        xSemaphoreGive(scale_config.scale_measurement_mutex);
    }
...
bool http_rest_scale_config(struct fs_file *file, int num_params, char *params[], char *values[]) {
    ...
    for (int idx = 0; idx < num_params; idx += 1) {
        if (strcmp(params[idx], "s0") == 0) {
            int driver_idx = atoi(values[idx]);
            validation = validate_scale_driver(driver_idx);
            if (!validation.is_valid) {
                return send_validation_error(file, validation.error_message);
            }
            set_scale_driver((scale_driver_t)driver_idx);
        }
        else if (strcmp(params[idx], "s1") == 0) {
            int baudrate_idx = atoi(values[idx]);
            validation = validate_scale_baudrate(baudrate_idx);
            if (!validation.is_valid) {
                return send_validation_error(file, validation.error_message);
            }
            scale_config.persistent_config.scale_baudrate = (scale_baudrate_t)baudrate_idx;
        }
    ...
    int len = snprintf(scale_config_to_json_buffer,
    ...
    CHECK_SNPRINTF_OVERFLOW(len, sizeof(scale_config_to_json_buffer), file);
