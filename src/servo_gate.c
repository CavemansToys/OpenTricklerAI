...
bool http_rest_servo_gate_state(struct fs_file *file, int num_params, char *params[], char *values[]) {
    ...
    for (int idx = 0; idx < num_params; idx += 1) {
        if (strcmp(params[idx], "g0") == 0) {
            ...
            // Validate state enum (0-2: GATE_DISABLED, GATE_CLOSE, GATE_OPEN)
            if (state_value < GATE_DISABLED || state_value > GATE_OPEN) {
                return send_validation_error(file, "Invalid gate state");
            }
            gate_state_t state = (gate_state_t) state_value;
            servo_gate_set_state(state, false);
        }
    }
    
    // Response
    int len = snprintf(servo_gate_json_buffer,
    ...
    CHECK_SNPRINTF_OVERFLOW(len, sizeof(servo_gate_json_buffer), file);
...
bool http_rest_servo_gate_config(struct fs_file *file, int num_params, char *params[], char *values[]) {
    ...
    for (int idx = 0; idx < num_params; idx += 1) {
        if (strcmp(params[idx], "c0") == 0) {
            ...
            validation = validate_servo_duty_cycle(duty_cycle);
            ...
            validation = validate_servo_speed(speed);
            if (!validation.is_valid) {
                return send_validation_error(file, validation.error_message);
            }
            servo_gate.eeprom_servo_gate_config.shutter_close_speed_pct_s = speed;
        }
            ...
            servo_gate.eeprom_servo_gate_config.shutter_open_speed_pct_s = speed;
        }
