...
bool http_rest_wireless_config(struct fs_file *file, int num_params, char *params[], char *values[]) {
    ...
    for (int idx = 0; idx < num_params; idx += 1) {
        if (strcmp(params[idx], "w0") == 0) {
            ...
            validation = validate_wireless_auth(auth);
            if (!validation.is_valid) {
                return send_validation_error(file, validation.error_message);
            }
            wireless_config.eeprom_wireless_metadata.auth = (cyw43_auth_t)auth;
        }
        else if (strcmp(params[idx], "w3") == 0) {
            uint32_t timeout_ms = (uint32_t) atoi(values[idx]);
            validation = validate_wireless_timeout(timeout_ms);
            if (!validation.is_valid) {
                return send_validation_error(file, validation.error_message);
            }
    ...
    CHECK_SNPRINTF_OVERFLOW(len, sizeof(wireless_config_json_buffer), file);
